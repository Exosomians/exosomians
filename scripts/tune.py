...
def train_fn(config):
    ...
    exo.ml.ExoCNN.setup_dataset(path='/home/mohsen/projects/exosomians-v2/data/design.mat.csv',
                                seq_key='seq',
                                target_key='label',
                                fraction=0.5)
...
config = {
    'n_conv_blocks': tune.choice([1, 2, 3]),
    'n_conv_layers': tune.choice([1, 2, 3]),
    'kernel_size': tune.choice([10, 8, 5, 3]),
    'n_filters': tune.choice([16, 32, 64]),
    'lr': tune.loguniform(1e-6, 1e-2),
    'pooling': tune.choice(['avg', 'max']),
    'pooling_size': tune.choice([2, 3, 4, 5]),
